...
class PointCloudToImage:
    ...
    def process_arm_image(self):
        ...
        if self.save:
            ...
            fig.savefig('EXP2/Arm/Original3d_arm' + str(self.count)+ '.png')
            np.save('EXP2/Arm/Original3d_arm' + str(self.count), self.point_matrix_arm)
        ...
        if self.save:
            ...
            fig.savefig('EXP2/Arm/Filtered3d_arm' + str(self.count)+ '.png')
            np.save('EXP2/Arm/Filtered3d_arm' + str(self.count),objects_points)
        ...
        if good_count>0:
            ...
            if self.save:
                ...
                fig.savefig('EXP2/Arm/2dRectangle_arm'+ str(self.count)+ '.png')
                np.save('EXP2/Arm/2dRectangle_arm'+ str(self.count),rectangle)
    ...
    def process_front_image(self):
        ...
        if self.save:
            ...
            name="EXP2/Front/Imagen con todos Yolo Objeto" + str(self.count)+ ".png"
        ...
        if best_result:
            ...
            if self.save:
                ...
                ax.set_box_aspect([np.ptp(objects_points[:,0]), np.ptp(objects_points[:,1]), np.ptp(objects_points[:,2])])
                ax.scatter(objects_points[:,0], objects_points[:,1], objects_points[:,2], marker='o')
                fig.savefig('EXP2/Front/NoFiltered3d_front' + str(self.count)+ '.png')
                np.save('EXP2/Front/NoFiltered3d_front' + str(self.count),objects_points)
                ...
                fig.savefig('EXP2/Front/Filtered3d_front' + str(self.count)+ '.png')
                np.save('EXP2/Front/Filtered3d_front' + str(self.count),filtered_points)
            ...
            if graspable_dim[0] < 0.12 and graspable_dim[0] > 0.01:
                ...
                if self.save:
                    ...
                    name="EXP2/Front/Imagen con Mejor Anotacion Y Gripper" + str(self.count)+ ".png"
                ...
                if self.save:
                    annotated_image = cv2.circle(annotated_image, (center_x, center_y), radius=5, color=(0, 0, 255), thickness=-1)
                    annotated_image = cv2.circle(annotated_image, (min_index[1], min_index[0]), radius=5, color=(0, 255, 0), thickness=-1)
                    name="EXP2/Front/Imagen con Mejor Anotacion Y Centro" + str(self.count)+ ".png"
                    cv2.imwrite(name, annotated_image)
                    cv2.waitKey(1000)
            ...
            with open("EXP2/Front/execution_times.txt", "a") as file:  
